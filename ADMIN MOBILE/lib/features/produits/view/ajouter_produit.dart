...
class _ProduitFormWidgetState extends State<ProduitFormWidget> {
  ...
  List<TypeProduit> _filteredTypes = []; // On stocke les objets TypeProduit pour avoir l'ID
  ...
  void _soumettre() {
    if (_formKey.currentState!.validate()) {
      final boutiqueservice = Provider.of<BoutiqueService>(context, listen: false);
    ...
    return Consumer<TypesProduitsViewModel>(
      builder: (context, typeProvider, child) {
        ...
        final typesDisponibles = _selectedGroupeId != null
            ? typeProvider.typesProduits.where((t) => t.groupeId == _selectedGroupeId).toList()
            : <TypeProduit>[];
        _filteredTypes = typesDisponibles
            .where((t) => t.nom.toLowerCase().contains(_rechercheTypeController.text.toLowerCase()))
            .toList();
  ...
  Widget _buildTypeField(TypesProduitsViewModel provider) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Type (optionnel)', style: TextStyle(fontWeight: FontWeight.bold)),
        const SizedBox(height: 4),
        GestureDetector(
            ...
            child: Row(
              ...
              children: [
                Expanded(
                  child: Text(
                    _selectedTypeId != null
                        ? provider.typesProduits.firstWhere((t) => t.id == _selectedTypeId).nom
                        : 'Sélectionner un type...',
                    overflow: TextOverflow.ellipsis,
                    style: TextStyle(
                      color: _selectedTypeId != null ? Colors.black : Colors.grey[600],
                    ),
                  ),
                ),
  ...
  void _showTypeBottomSheet(BuildContext context, TypesProduitsViewModel provider) {
        ...
        return StatefulBuilder(
          builder: (context, setState) {
            ...
            return Container(
              ...
              child: Column(
                children: [
                  const Text('Sélectionner un type', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 16),
                  TextField(
                    ...
                    child: displayedTypes.isEmpty
                        ...
                        : ListView.builder(
                            ...
                            itemBuilder: (ctx, index) {
                              ...
                              return ListTile(
                                title: Text(type.nom),
                                trailing: _selectedTypeId == type.id ? const Icon(Icons.check, color: Colors.blue) : null,
                                onTap: () {
                                  setState(() {
                                    _selectedTypeId = type.id;
                                    _rechercheTypeController.text = type.nom;
                                  });
  ...
  // void _showCreateTypeDialog(BuildContext context, TypesProduitsViewModel provider) {
  ...
  //           ElevatedButton(
  //             onPressed: () async {
  //               if (nomController.text.isNotEmpty && _selectedGroupeId != null) {
  //                 // Appeler le provider pour ajouter le type
  //                 await provider.addTypeProduit(nomController.text, _selectedGroupeId!);
  //                 if (mounted) {
  //                   // Mettre à jour la sélection avec le nouvel ID
  //                   final nouveauType = provider.typesProduits.lastWhere((t) => t.nom == nomController.text);
  //                   setState(() {
  //                     _selectedTypeId = nouveauType.id as int?;
              ...
              children: [
                Expanded(
                  child: Text(
                    ...
                    overflow: TextOverflow.ellipsis,
                    style: TextStyle(
                      color: _selectedMarque != null ? Colors.black : Colors.grey[600],
                    ),
                  ),
                ),
                Icon(Icons.arrow_drop_down, color: Colors.grey[600]),
        ...
        return StatefulBuilder(
          builder: (context, setState) {
            ...
            return Container(
              ...
              child: Column(
                children: [
                  const Text('Sélectionner une marque', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 16),
                  TextField(
                    controller: searchController,
                    decoration: const InputDecoration(
                    ...
                    child: displayedMarques.isEmpty
                        ...
                        : ListView.builder(
                            ...
                            itemBuilder: (ctx, index) {
                              ...
                              return ListTile(
                                title: Text(marque),
                                trailing: _selectedMarque == marque ? const Icon(Icons.check, color: Colors.blue) : null,
                                onTap: () {
                                  setState(() {
          ...
          child: TextButton(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 8),
                const Text('Ajouter une image', style: TextStyle(color: Colors.blue)),
              ],
            ),
          ),
        ),
